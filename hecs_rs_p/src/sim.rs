...
struct PositionUpdateItem {
    role: Role,
    start_sec: i64,
    route: *const Vec<RoutePoint>,
    segment_end_secs: *const Vec<f64>,
    total_duration_sec: f64,
    position: *mut Ecef,
}

// 生ポインタはスレッド安全性を自動で保証できないため、
// ここでは「同じ位置を二重に指さない」前提で並列更新できると判断し、
// 位置更新専用の要素として Sync を明示的に付与します。
unsafe impl Sync for PositionUpdateItem {}
...
pub fn position_update_system(
    ...
    for (_entity, (role, start_sec, route, segment_end, total_duration, position)) in query.iter() {
        items.push(PositionUpdateItem {
            role: *role,
            start_sec: start_sec.0,
            route: &route.0 as *const Vec<RoutePoint>,
            segment_end_secs: &segment_end.0 as *const Vec<f64>,
            total_duration_sec: total_duration.0,
            position: position as *mut Ecef,
        });
    }

    // 並列化の粒度を調整してスレッド起動の負担を減らすため、
    // 連続したスナップショットを一定数ずつのチャンクに分けて同時更新します。
    // 計測しやすいように環境変数でチャンクサイズを変えられるようにします。
    ...
    items.par_chunks(chunk_size).for_each(|chunk| {
        for item in chunk.iter() {
            // ここではECSの各コンポーネントを「読み取り専用の参照」と
            // 「位置だけを指す生ポインタ」に分けて持っています。
            // 同じエンティティの位置を二重に指すことはないので、
            // 並列で書き換えてもデータ競合が起きない前提です。
            let route = unsafe { &*item.route };
            let segment_end_secs = unsafe { &*item.segment_end_secs };
            let next_pos = position_at_time(
                item.role,
                item.start_sec,
                route,
                segment_end_secs,
