...
func runSimulation(scenarioPath, timelinePath, eventPath string) error {
	// AoSは「1つのオブジェクトに必要な情報をまとめて持つ」方式なので、
	// 基本は配列を順番に更新しますが、ここでは位置更新だけを並列化して性能を測ります。
	...
	positionsScratch := make([]TimelinePosition, 0, len(objects))
	chunkSize := resolveChunkSize()
	parallelism := resolveParallelism()
	positionUpdater := NewPositionUpdater(objects, chunkSize, parallelism)
	defer positionUpdater.Close()
	...
	for timeSec := 0; timeSec <= endSec; timeSec++ {
		// まず全オブジェクトの位置を更新します。
		// 並列化の粒度を調整するためにチャンクサイズと並列数を外から変えられるようにします。
		positionUpdater.Update(float64(timeSec))

		// 空間ハッシュを作り、近傍探索を高速化します。
		spatialHash = buildSpatialHash(objects, detectRange, spatialHash)

		// 斥候の探知イベントを判定してイベントログへ出力します。
		if err := emitDetectionEvents(timeSec, detectRange, spatialHash, objects, eventEncoder); err != nil {
			return err
		}

		// 攻撃役の爆破イベントを判定してイベントログへ出力します。
		if err := emitDetonationEvents(timeSec, bomRange, objects, eventEncoder); err != nil {
			return err
		}

		// 全オブジェクトの位置をタイムラインログへ出力します。
		var err error
		positionsScratch, err = emitTimelineLog(timeSec, objects, positionsScratch, timelineEncoder)
		if err != nil {
			return err
		}
	}
	...
	fmt.Fprintf(os.Stderr, "aos_go: simulation elapsed: %fs\n", time.Since(start).Seconds())
