...
func runSimulation(scenarioPath, timelinePath, eventPath string) error {
	...
	world, ecs, positionUpdater := buildWorldAndECS(scenario)
	...
	fmt.Fprintf(os.Stderr, "donburi_go: simulation elapsed: %fs\n", time.Since(start).Seconds())

	return nil
}

func buildWorldAndECS(scenario *Scenario) (donburiWorld, *ecsLib.ECS, *PositionUpdater) {
	world := newWorld()
	...
	positionUpdater := NewPositionUpdater(world, chunkSize, parallelism)

	ecs.AddSystem(makePositionUpdateSystem(positionUpdater))
	ecs.AddSystem(snapshotSystem)
	ecs.AddSystem(detectionSystem)
	ecs.AddSystem(detonationSystem)
	ecs.AddSystem(timelineSystem)

	return world, ecs, positionUpdater
}
