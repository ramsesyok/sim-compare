...
func makePositionUpdateSystem(updater *PositionUpdater) func(*ecsLib.ECS) {
	return func(ecs *ecsLib.ECS) {
		// donburiのSystemは関数として定義し、並列更新は外部の更新器に任せます。
		timeSec := getTimeSec(ecs.World)
		updater.Update(float64(timeSec))
	}
}
