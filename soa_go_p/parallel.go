...
func NewPositionUpdater(state *SoaState, chunkSize int, workerCount int) *PositionUpdater {
	if chunkSize <= 0 {
		...
		go func() {
			...
			for job := range updater.jobs {
				for index := job.start; index < job.end; index++ {
					updater.state.Positions[index] = positionAtTime(updater.state, index, job.timeSec)
				}
				job.wait.Done()
			}
		}()
...
func (u *PositionUpdater) Update(timeSec float64) {
	if len(u.state.IDs) == 0 {
		return
	}

	var wg sync.WaitGroup
	for start := 0; start < len(u.state.IDs); start += u.chunkSize {
		end := start + u.chunkSize
		if end > len(u.state.IDs) {
			end = len(u.state.IDs)
		}
		wg.Add(1)
