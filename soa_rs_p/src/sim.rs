...
struct SoaStateView<'a> {
    roles: &'a [Role],
    start_secs: &'a [i64],
    routes: &'a [Vec<RoutePoint>],
    segment_end_secs: &'a [Vec<f64>],
    total_duration_secs: &'a [f64],
}

pub fn update_positions(state: &mut SoaState, time_sec: f64) {
    let view = SoaStateView {
        roles: &state.roles,
        start_secs: &state.start_secs,
        routes: &state.routes,
        segment_end_secs: &state.segment_end_secs,
        total_duration_secs: &state.total_duration_secs,
    };
    // 並列化の粒度を調整してスレッド起動の負担を減らすため、
    // 連続した位置配列を一定数ずつのチャンクに分けて同時更新します。
    ...
    state
        ...
        .for_each(|(chunk_index, chunk)| {
            ...
            for (offset, position) in chunk.iter_mut().enumerate() {
                ...
                *position = position_at_time(&view, index, time_sec);
            }
        });
}

fn position_at_time(state: &SoaStateView<'_>, index: usize, time_sec: f64) -> Ecef {
