...
func (s *PositionUpdateSystem) Initialize(w *ecs.World) {
	...
	count := query.Count()
	s.entities = make([]ecs.Entity, count)
	for i := 0; i < count; i++ {
		s.entities[i] = query.EntityAt(i)
	}
	query.Close()

	chunkSize := resolveChunkSize()
	parallelism := resolveParallelism()
	s.updater = NewPositionUpdater(w, s.entities, chunkSize, parallelism)
}

func (s *PositionUpdateSystem) Update(w *ecs.World) {
	time := s.timeRes.Get().TimeSec
	s.updater.Update(float64(time))
}
